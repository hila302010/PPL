...
// Support parsing for ‘class’ special form :
// ClassExp -> (class <varDecl> <binding>)
export const parseClass = (params: List<Sexp>): Result<ClassExp> =>
    isNonEmptyList<Sexp>(params) ? 
        isEmpty(rest(params)) ? makeFailure(`class missing 1 arguments: ${format(params)}`) :
        (params.length !== 2) ? makeFailure(`class should have 2 arguments: ${format(params)}`) :
        parseGoodClass(first(params), second(params)) :
    makeFailure("Class missing 2 arguments- fields and methods");

const parseGoodClass = (fields: Sexp, methods: Sexp): Result<ClassExp> =>{
    ...
    if (isArray(fields) && allT(isString, fields)&& isOk(bindingsResult)){
        return makeOk(makeClassExp(map(makeVarDecl, fields), bindingsResult.value));
    }
    return makeFailure("Sexp is not valid");
}
