...
export type ExtProcTExp = ProcTExp | PredicateTExp;
export const isExtProcTExp = (x: any): x is ExtProcTExp =>
    isProcTExp(x) || isPredicateTExp(x);

export type CompoundTExp = TupleTExp | UnionTExp | InterTExp | ExtProcTExp | PredicateResultTExp;
export const isCompoundTExp = (x: any): x is CompoundTExp => isProcTExp(x) || isTupleTExp(x) || isUnionTExp(x)  || isInterTExp(x) || isPredicateResultTExp(x) || isExtProcTExp(x);
...
// TODO
// inter(any, number) - union(boolean. number) => never
// Helper function to check if one TExp is not a subtype of another

// Function to compute the difference between two type expressions
export const makeDiffTExp = (te1: TExp, te2: TExp): TExp =>
    isAtomicTExp(te1) ? isSubType(te1, te2) ? makeNeverTExp() : te1 :
    isUnionTExp(te1) && isUnionTExp(te2) ?
        makeUnionTExp(flattenUnion(te1.components).filter(
            (comp: TExp) => !containsType(flattenUnion(te2.components), comp))) :
    isUnionTExp(te1) ? makeUnionTExp(flattenUnion(te1.components).map(comp => makeDiffTExp(comp, te2))) :
    //check
    isInterTExp(te1) && isUnionTExp(te2) ? makeInterTExp(flattenInter(te1.components).filter(
        (comp: TExp) => !containsType(flattenInter(te2.components), comp) && !isAnyTExp(comp))) :
    isUnionTExp(te2) ? makeUnionTExp(flattenUnion(te2.components).map(comp => makeDiffTExp(te1, comp))) :
    isInterTExp(te1) && isInterTExp(te2) ? makeInterTExp(flattenInter(te1.components).filter(
        (comp: TExp) => !containsType(flattenInter(te2.components), comp))) :
    te1;


export type PredicateTExp = { tag: "PredicateTExp"; predProc: ProcTExp;};
export const makePredicateTExp = (pred: ProcTExp): PredicateTExp =>
    ({tag: "PredicateTExp", predProc: pred})
export const isPredicateTExp = (x: any): x is PredicateTExp => x.tag === "PredicateTExp";

export type PredicateResultTExp = { tag: "PredicateResultTExp"; value: TExp;};
export const makePredicateResultTExp = (val: TExp): PredicateResultTExp =>
    ({tag: "PredicateResultTExp", value: val})
export const isPredicateResultTExp = (x: any): x is PredicateResultTExp => x.tag === "PredicateResultTExp";
...
// [number, union(number, string)] => [number, number, string]
// [union(union(bool, num), string)] => [union(bool, num), string]
...
// Flatten all inter components into the result, excluding `AnyTExp` components
// [number, inter(any, string)] => [number, string]
const flattenInter = (tes: TExp[]): TExp[] => 
    (tes.length > 0) ? 
        isAnyTExp(tes[0]) ? [...flattenInter(tes.slice(1))] :
        isInterTExp(tes[0]) ? [...tes[0].components, ...flattenInter(tes.slice(1))] :
        [tes[0], ...flattenInter(tes.slice(1))] :
    [];
...
const subTypeComparator = (te1: TExp, te2: TExp): number =>
    equals(te1, te2) ? 0 :
    isSubType(te2, te1) ? 1 :  // +1 if te1 > te2
    isSubType(te1, te2) ? -1 : // -1 if te1 < te2
...
// ((a+b), (c+d), (d+e+f)) x gh -> (ac + ad + bc + bd) x (d+e+f) x gh
// (acd + ace + acf + add + ade + adf + bcd + bce + bcf + bdd + bde + bdf) x gh
// now for each product we add  gh
...
// (te1: (union number boolean), te2: number) => false
// (te1: number, te2: (union number boolean)) => true
// SubType comparator
export const isSubType = (te1: TExp, te2: TExp): boolean =>
    ...
    (isInterTExp(te1) && isUnionTExp(te2)) ? isSubset(te1.components.filter((component : TExp) => !isAnyTExp(component)), te2.components) : 
    (isUnionTExp(te1) && isInterTExp(te2)) ? isSubset(te1.components, te2.components) :
    isUnionTExp(te2) ? containsType(te2.components, te1) :
    (isAtomicTExp(te1) && isAtomicTExp(te2)) ? equalsTExp(te1, te2) :
    (isAtomicTExp(te2) && isPredicateResultTExp(te1)) ? equals(te1.value, te2) :
    (isPredicateTExp(te1) && isPredicateTExp(te2)) ? checkProcTExps(te1.predProc, te2.predProc) :
    (isPredicateResultTExp(te1) && isPredicateResultTExp(te2)) ? isSubType(te1.value, te2.value) :
    isPredicateResultTExp(te2) ? isSubType(te1, te2.value) :
    (isProcTExp(te1) && isProcTExp(te2)) ? checkProcTExps(te1, te2) :
    isTVar(te1) ? equals(te1, te2) : false;

const equalsTExp = (te1 : TExp, te2: TExp) : boolean =>
    te1.tag === te2.tag



// tes: number boolean, te: number  => true
...
// Expect (union texp1 ...)
// const parseisTypePredTExp = (texps: Sexp[]): Result<TExp> =>
//     mapv(mapResult(parseTExp, texps.slice(1)),
//          (tes: TExp[]) => makeisTypePredTExp(tes));
...
export const parsePredResExp = (texp: Sexp): Result<TExp> => 
    (texp === "number") ? makeOk(makePredicateResultTExp(makeNumTExp())) :
    (texp === "boolean") ? makeOk(makePredicateResultTExp(makeBoolTExp())) :
    (texp === "void") ? makeOk(makePredicateResultTExp(makeVoidTExp())) :
    (texp === "string") ? makeOk(makePredicateResultTExp(makeStrTExp())) :

    (texp === "any") ? makeOk(makePredicateResultTExp(makeAnyTExp())) :
    (texp === "never") ? makeOk(makePredicateResultTExp(makeNeverTExp())) :
    isString(texp) ? makeOk(makePredicateResultTExp(makeTVar(texp))) :
    isArray(texp) ? parseCompoundTExp(texp)
    : makeFailure("not valid")

export const parsePredExp = (texps: Sexp[]): Result<TExp> => {
    const pos = texps.indexOf('->');
    return (pos === -1)  ? makeFailure(`Procedure type expression without -> - ${format(texps)}`) :
           (pos === 0) ? makeFailure(`No param types in proc texp - ${format(texps)}`) :
           (pos === texps.length - 1) ? makeFailure(`No return type in proc texp - ${format(texps)}`) :
           ...
           bind(parseTupleTExp(texps.slice(0, pos)), (args: TExp[]) =>
               mapv(parsePredResExp(texps[pos + 2]), (returnTE: TExp) =>
                    makePredicateTExp(makeProcTExp(args, returnTE))));
};
    ...
    const up = (x?: TExp): Result<string | string[]> =>
        ...
        isProcTExp(x) ? bind(unparseTuple(x.paramTEs), (paramTEs: string[]) =>
                            mapv(unparseTExp(x.returnTE), (returnTE: string) =>
                                [...paramTEs, '->', returnTE])) :
        isPredicateResultTExp(x) ? bind(unparseTExp(x.value), (value: string) => makeOk(`(is? ${value})`)) :
        isPredicateTExp(x) ? unparseTExp(x.predProc) :
