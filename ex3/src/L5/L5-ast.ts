...
import { isTVar, makeFreshTVar, parseTExp, unparseTExp, TExp , parsePredExp, parsePredResExp} from './TExp';
...
// export type ExtProcExp = ProcExp | PredicateExp;
// export const isExtProcExp = (x: any): x is ExtProcExp =>
//     isProcExp(x) || isPredicateExp(x);
...
// L52
// (lambda (<vardecl>*) [: [is?] ? <TExp>]  ? <cexp>+ ) / ProcExp(args:VarDecl[], body:CExp[], returnTE: TExp))
const parseProcExp = (vars: Sexp, rest: Sexp[]): Result<ProcExp> => {
    if (isArray(vars)) {
        const args = mapResult(parseVarDecl, vars);
        const body = mapResult(parseL5CExp, rest[0] === ":" && rest[1] === "is?" ? rest.slice(3) : rest[0] === ":" ? rest.slice(2) : rest);
        const returnTE = rest[0] === ":"  && rest[1] === "is?" ?  parsePredResExp(rest[2]) :
...
// In L52 the varDecl as TExp for the value
export const parseVarDecl = (sexp: Sexp): Result<VarDecl> => {
    if (isString(sexp)) {
        ...
        const v = sexp[0];
        if (isString(v)) {
            return mapv(((isArray(sexp[2]) && sexp[2][2] === "is?") ? 
            parsePredExp(sexp[2]) : parseTExp(sexp[2])), (te: TExp) => makeVarDecl(v, te));
        } else {
